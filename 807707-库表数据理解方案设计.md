...
根据设计稿，为 `form_view_field` 表增加三个字段：

```sql
-- 为 form_view_field 表增加三个字段
ALTER TABLE form_view_field
ADD COLUMN IF NOT EXISTS field_role TINYINT DEFAULT NULL COMMENT '字段角色：1-业务主键, 2-关联标识, 3-业务状态, 4-时间字段, 5-业务指标, 6-业务特征, 7-审计字段, 8-技术字段' AFTER field_business_name,
...
3. **参数过滤处理**
   ...
   *状态为 0-未理解、3-已完成、4-已发布时（查询正式表）：*
   ...
   -- 在 WHERE 子句中添加条件
   AND (
     field_business_name IS NULL OR field_business_name = ''
...
3. **使用数据库事务执行数据同步**
   ...
   **事务开始：**
   ...
   ```sql
   -- 逻辑删除原有业务对象
   UPDATE t_business_object
   SET deleted_at = NOW(3)
   WHERE form_view_id = {form_view_id} AND deleted_at IS NULL;

   -- 逻辑删除原有业务对象属性
   UPDATE t_business_object_attributes
   SET deleted_at = NOW(3)
   WHERE form_view_id = {form_view_id} AND deleted_at IS NULL;
   ```
   ...
   INSERT INTO t_business_object (
     id, uuid, object_name, object_type, form_view_id, status, created_at, updated_at, deleted_at
   )
   SELECT
     bot.id, bot.uuid, bot.object_name, 1 AS object_type, bot.form_view_id, 1 AS status,
     NOW(3), NOW(3), 0
   FROM t_business_object_temp bot
   WHERE bot.form_view_id = {form_view_id}
     AND bot.version = {current_version}
   ...
   SELECT
     boattr.id, boattr.form_view_id, bo.uuid AS business_object_id,
     boattr.form_view_field_id, boattr.attr_name, NOW(3), NOW(3), NULL
   FROM t_business_object_attributes_temp boattr
   JOIN t_business_object_temp bot ON boattr.business_object_id = bot.id
   WHERE boattr.form_view_id = {form_view_id}
     AND boattr.version = {current_version}
     AND boattr.deleted_at IS NULL;
   ```
...
4. **调用 AI 服务重新识别业务对象**
   ...
   发送 Kafka 消息到 `data-understanding-requests` 主题：
...
5. **异步处理 AI 返回结果**
   ...
   订阅 `data-understanding-responses` 主题，接收 AI 分析结果：
...
6. **保存新的业务对象数据**
   ...
   **插入新版本的业务对象数据**：
   ...
   INSERT INTO t_business_object_temp (
     id, form_view_id, user_id, version, object_name, created_at, updated_at, deleted_at
   )
   VALUES
     ({uuid}, {form_view_id}, NULL, {new_version}, '基础信息', NOW(3), NOW(3), NULL);
   ```
   ...
   **插入新版本的属性数据**：
   ...
   INSERT INTO t_business_object_attributes_temp (
     id, form_view_id, business_object_id, user_id, version, form_view_field_id, attr_name, created_at, updated_at, deleted_at
   )
   VALUES
     ({uuid}, {form_view_id}, {business_object_id}, NULL, {new_version}, {form_view_field_id}, '姓名', NOW(3), NOW(3), NULL);
   ```
7. **更新理解状态（如果当前是已完成状态）**

   ```sql
   -- 如果当前状态是3-已完成，更新为2-待确认
   UPDATE form_view
   SET understand_status = 2
   WHERE id = {form_view_id} AND understand_status = 3;
   ```
8. **说明**：

   - 旧版本记录保留，通过版本号区分
   - 新版本记录的 `user_id` 为 NULL，表示 AI 模型操作
   - 版本号递增
   - 如果当前状态是 `3-已完成`，重新识别后将状态更新为 `2-待确认`
   - 如果当前状态是 `2-待确认`，保持状态不变
...
3. **收集库表和字段基础数据**
   ...
   SELECT
     id AS form_view_field_id,
     technical_name AS field_tech_name,
     business_name AS field_business_name,
     data_type AS field_type,
     comment AS field_comment,
     primary_key AS is_primary_key,
     `index` AS field_index
   FROM form_view_field
   WHERE form_view_id = {form_view_id}
     AND deleted_at IS NULL
   ORDER BY `index` ASC;
...
5. **异步执行 AI 分析任务**
   ...
   **5.1 发送 Kafka 请求消息到 `data-understanding-requests` 主题**
   ...
   **5.2 订阅 Kafka 响应消息 `data-understanding-responses` 主题**
...
6. **保存 AI 返回的数据到临时表**
   ...
   **6.1 保存库表信息到 t_form_view_info_temp**
   ...
   **6.2 保存字段信息到 t_form_view_field_info_temp**
   ...
   **6.3 保存业务对象到 t_business_object_temp**
   ...
   **6.4 保存业务对象属性到 t_business_object_attributes_temp**
   ...
   VALUES
     ...
     ({uuid_3}, {form_view_id}, {uuid_2}, NULL, {new_version}, 'field-uuid-4', '手机号',
      NOW(3), NOW(3), NULL);
   ```
7. **【AI完成后】修改 form_view 表中 understand_status 字段为 `2-待确认`**
...
8. **Kafka 消费者处理流程**
   ...
   **消费者处理逻辑**：
   ...
   public void handleAIResponse(String message) {
       ...
       // 3. 保存 AI 返回的数据到临时表（使用事务）
       try {
           transactionTemplate.execute(status -> {
               saveTableSemantic(response.getTableSemantic());
               saveFieldsSemantic(response.getFieldsSemantic());
               saveBusinessObjects(response.getBusinessObjects());
               return null;
           });

           // 4. 更新 understand_status 为 2-待确认
           updateUnderstandStatus(response.getFormViewId(), 2);

           // 5. 标记消息已处理
           markMessageAsProcessed(response.getMessageId());

       } catch (Exception e) {
           log.error("处理 AI 响应失败: {}", response.getMessageId(), e);
           // 不重试，记录错误日志
           // understand_status 保持为 1-理解中，需要人工介入处理
       }
   }
   ```
...
9. **异常处理**
   ...
   | ----------------- | --------------------------------------------------------- | ------------------- |
   | AI服务不可用/超时 | 提示"服务器繁忙，请稍后再试"                              | 回退到 `0-未理解` |
   | AI分析失败        | 记录错误日志到 t_kafka_message_log，状态保持 `1-理解中` | 保持 `1-理解中`   |
   | Kafka发送失败     | 记录错误日志，回退状态到 `0-未理解`                     | 回退到 `0-未理解` |
   | 数据保存失败      | 记录错误日志到 t_kafka_message_log，状态保持 `1-理解中` | 保持 `1-理解中`   |
...
- `1-理解中` → `2-待确认`：AI异步处理完成后自动设置，前端需要轮询或接收通知来获取状态变化
