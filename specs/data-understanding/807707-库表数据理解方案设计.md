...
2. **获取 form_view 数据（库表和字段信息）**
   ...
   **2.3 获取字段信息（技术信息 + 语义信息）**
   ...
   SELECT
     ...
     fvf.comment AS form_view_field_comment
   FROM form_view_field fvf
   LEFT JOIN t_form_view_field_info_temp fvfit
     ON fvf.id = fvfit.form_view_field_id
     AND fvfit.version = {current_version}
   ...
   **3.2 获取字段信息（两种方式）**
   ...
   - **方式二：未传 fields 参数（全部字段理解）**
     ...
     SELECT
       ...
       comment AS form_view_field_comment
     FROM form_view_field
     WHERE form_view_id = {form_view_id}
       AND deleted_at IS NULL
     ORDER BY `index` ASC;
...
**接口路径**：`POST /api/v1/data-semantic/batch-object-match`
...
| entries[].data_source.id   | string | 否   | 视图ID（对应form_view表mdl_id） |
...
4. **Step 3 - 视图表模糊匹配**：
   ```sql
   SELECT id, understand_status, technical_name, business_name, mdl_id
   FROM form_view
   WHERE business_name LIKE '%{name}%' AND deleted_at IS NULL;
   ```
5. **Step 4 - 状态检查**：
